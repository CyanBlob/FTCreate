...
use druid::widget::{Button, Container, Flex, Label, List, Padding, ViewSwitcher};
use druid::{AppLauncher, Data, Lens, LensExt, PlatformError, Widget, WidgetExt, WindowDesc};
...
fn build_ui() -> impl Widget<AppState> {
    Padding::new(
        ...
        Flex::column()
            ...
            .with_flex_child(
                Button::new("Update").on_click(|_, data, _| {
                    println!("{:?}", data);
                }),
                1.0,
            ),
...
fn render_generator<T: MotorGenerator>() -> impl Widget<AppState> {
    Flex::column().with_flex_child(
        ...
        .lens(AppState::drivetrain.then(Drivetrain::motors)),
        1.0,
    )
}
